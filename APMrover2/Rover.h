...
private:
    ...
    RC_Channel *channel_throttle_d;
    RC_Channel *channel_learn;



    RC_Channel *channel_throttle_dir;
    RC_Channel *channel_steer_dir;
...
private:
    ...
    void send_Ctrl_CMD_to_EtherCAT(AP_HAL::UARTDriver *uart , int16_t throttle, int16_t steer);
    void EtherCAT_data_exchange(void);
    bool EtherCAT_FeedBack_read(void);
    void EtherCAT_camera_send(uint8_t data_type , const uint8_t camera_command[]);
    void setup_CWE(void);
    void clean_array(uint8_t array[], uint8_t num);
    void gcs_data_exchange(void);
    void send_boat_status(mavlink_channel_t chan);
    void mid_adjust(void);
    void steer_pos_output(double steer_pos_tar);
