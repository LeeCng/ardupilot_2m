...
if (fabsf(Nu) > Nu_limit && fabsf(_last_Nu) > Nu_limit
...
//            _groundspeed_vector.y);
//    hal.uartE->printf("A_air.x: %f A_air.y : %f AB.x： %f AB.y: %f \n", A_air.x,
//            A_air.y, AB.x, AB.y);
//
//}
...
float latAccDemCircCtr = velTangent
        * velTangent/ max((0.5f * radius), (radius + xtrackErrCirc));
...
if (xtrackErrCirc > 0.0f
        && loiter_direction * latAccDemCap < loiter_direction * latAccDemCirc) {
