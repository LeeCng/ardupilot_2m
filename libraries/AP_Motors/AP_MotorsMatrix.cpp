...
static void test_uart(AP_HAL::UARTDriver *uart, const char *name, int16_t yaw, int16_t throttle_radio_output)
{
    if (uart == NULL) {
        // that UART doesn't exist on this platform
        return;
    }
    uart->printf("Yaw is %d at %.3f seconds, ",
                 yaw, hal.scheduler->millis()*0.001f);
    uart->printf("Throttle is %d at %.3f seconds. output by UART %s.\n",
            throttle_radio_output, hal.scheduler->millis()*0.001f, name);
...
void AP_MotorsMatrix::output_armed_stabilizing()
{
    ...
    */
    ...
    /*
        ...
        if(hal.scheduler->millis() % 100 <= 10)
        {
            //test_uart(hal.uartA, "uartA", yaw_pwm, throttle_radio_output);
            //test_uart(hal.uartB, "uartB", yaw_pwm, throttle_radio_output);
            //test_uart(hal.uartC, "uartC", yaw_pwm, throttle_radio_output);
            //test_uart(hal.uartD, "uartD", yaw_pwm, throttle_radio_output);
            //test_uart(hal.uartE, "uartE", yaw_pwm, throttle_radio_output);
