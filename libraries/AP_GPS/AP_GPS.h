...
public:
    ...
    struct GPS_State {
        ...
        float vertical_accuracy;bool have_vertical_velocity :1; ///< does this GPS give vertical velocity?
        bool have_speed_accuracy :1;bool have_horizontal_accuracy :1;bool have_vertical_accuracy :1;
...
#	define _GPS_TIMING(instance) timing[instance]
    ...
    const float &yaw(uint8_t instance) const {
        return _GPS_STATE(instance).yaw;
    }
    ...
    // 俯仰角  pitch
    const float &pitch(uint8_t instance) const {
        return _GPS_STATE(instance).pitch;
    }
    const float &pitch() const {
        return pitch(primary_instance);
    }

    // 滚转角  roll
    const float &roll(uint8_t instance) const {
        return _GPS_STATE(instance).roll;
    }
    const float &roll() const {
        return roll(primary_instance);
    }

    /************** 姿态角速度*************/
    // 航向角速度 yaw_rate
    const float &yaw_rate(uint8_t instance) const {
        return _GPS_STATE(instance).yaw_rate;
    }
    const float &yaw_rate() const {
        return yaw_rate(primary_instance);
    }

    // 俯仰角速度  pitch_rate
    const float &pitch_rate(uint8_t instance) const {
        return _GPS_STATE(instance).pitch_rate;
    }
    const float &pitch_rate() const {
        return pitch_rate(primary_instance);
    }

    // 滚转角速度  roll_rate
    const float &roll_rate(uint8_t instance) const {
        return _GPS_STATE(instance).roll_rate;
    }
    const float &roll_rate() const {
        return roll_rate(primary_instance);
    }

    /************** 加速度*************/
    // x方向 角加速度
    const float &acc_x(uint8_t instance) const {
        return _GPS_STATE(instance).acc_x;
    }
    const float &acc_x() const {
        return acc_x(primary_instance);
    }

    // y方向 角加速度
    const float &acc_y(uint8_t instance) const {
        return _GPS_STATE(instance).acc_y;
    }
    const float &acc_y() const {
        return acc_y(primary_instance);
    }

    // z方向 角加速度
    const float &acc_z(uint8_t instance) const {
        return _GPS_STATE(instance).acc_z;
    }
    const float &acc_z() const {
        return acc_z(primary_instance);
    }

    /************** 相对位置 *************/
    // x方向 相对位置
    const int32_t &RelPx(uint8_t instance) const {
        return _GPS_STATE(instance).RelPx;
    }
    const int32_t &RelPx() const {
        return RelPx(primary_instance);
    }

    // y方向 相对位置
    const int32_t &RelPy(uint8_t instance) const {
        return _GPS_STATE(instance).RelPy;
    }
    const int32_t &RelPy() const {
        return RelPy(primary_instance);
    }

    // z方向 相对位置
    const int32_t &RelPz(uint8_t instance) const {
        return _GPS_STATE(instance).RelPz;
    }
    const int32_t &RelPz() const {
        return RelPz(primary_instance);
    }
