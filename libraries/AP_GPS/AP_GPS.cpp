...
const uint32_t AP_GPS::_baudrates[] PROGMEM = { 4800U, 38400U, 115200U, 57600U,
    9600U, 230400U };

// initialisation blobs to send to the GPS to try to get it into the
// right mode
const prog_char AP_GPS::_initialisation_blob[] PROGMEM
=    UBLOX_SET_BINARY MTK_SET_BINARY SIRF_SET_BINARY;
const prog_char AP_GPS::_initialisation_raw_blob[] PROGMEM
 ...
 */
void AP_GPS::send_blob_start(uint8_t instance, const prog_char *_blob,
        uint16_t size) {
...
void AP_GPS::send_blob_update(uint8_t instance) {
    ...
    if (initblob_state[instance].remaining > 0) {
        ...
        while (space > 0) {
            _port[instance]->write(
                    pgm_read_byte(initblob_state[instance].blob));
...
//    }
    ...
    _port[instance]->set_flow_control(AP_HAL::UARTDriver::FLOW_CONTROL_DISABLE);
    dstate->last_baud_change_ms = now;
    _port[instance]->printf("baudrate_set:  %d   \n", baudrate);
    send_blob_start(instance, _initialisation_blob,
            sizeof(_initialisation_blob));

    send_blob_update(instance);

    _port[instance]->printf("before new_gps   \n");

    while (initblob_state[instance].remaining == 0
            && _port[instance]->available() > 0 && new_gps == NULL) {
...
void AP_GPS::setHIL(uint8_t instance, GPS_Status _status,
    ...
    uint64_t gps_time_ms = time_epoch_ms
            - (17000ULL * 86400ULL + 52 * 10 * 7000ULL * 86400ULL - 15000ULL);
    istate.time_week = gps_time_ms / (86400 * 7 * (uint64_t) 1000);
    istate.time_week_ms = gps_time_ms
            - istate.time_week * (86400 * 7 * (uint64_t) 1000);
    timing[instance].last_message_time_ms = tnow;
    timing[instance].last_fix_time_ms = tnow;
...
void AP_GPS::send_mavlink_gps_raw(mavlink_channel_t chan) {
    ...
    mavlink_msg_gps_raw_int_send(chan, last_fix_time_ms(0) * (uint64_t) 1000,
            ...
            loc.alt * 10UL, // in mm
            get_hdop(0), get_vdop(0), ground_speed(0) * 100,  // cm/s
